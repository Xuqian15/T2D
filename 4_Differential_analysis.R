...
if (TRUE){
  option_list = list(
    ...
    make_option(c("-e", "--exposure"), type="character", default="T2DM",
...
write.csv(meta_results,file = paste(opts$workplace,opts$output,"_differential_significance.csv",sep=''))
write.csv(maaslin_results,file = paste(opts$workplace,opts$output,"_differential_significance_single_cohort.csv",sep=''))
...
write.csv(feat_diff,file = paste(opts$workplace,opts$output,"_differential_signature.csv",sep=''))
